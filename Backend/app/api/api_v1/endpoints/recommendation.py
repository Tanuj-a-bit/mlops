...
from app.monitoring import (
    ...
    record_latency, 
    track_error,
    track_empty_response,
    record_item_count,
    track_recommendation_type
)
...
def load_model_from_db():
    ...
    try:
        ...
        query_items = 'SELECT p.id as itemid, p.name, p."categoryId" as categoryid, c.name as category_name FROM "Product" p LEFT JOIN "Category" c ON p."categoryId" = c.id'
        ...
        query_events = 'SELECT "visitorId" as visitorid, "itemId" as itemid, "eventType" as event, timestamp FROM "Event"'
...
def get_user_recommendations(user_id: int, limit: int = 5) -> List[RecommendedItem]:
    ...
    # Sort by timestamp, look at last 10 items instead of 3
    last_items = user_events.sort_values('timestamp', ascending=False)['itemid'].unique()[:10]
    recommendations = []
    
    # Calculate how many similiar items to fetch per history item to reach the target globally
    # Add a buffer (+2) to account for duplicates we filters later
    if len(last_items) > 0:
        per_item_limit = (limit // len(last_items)) + 5
    else:
        per_item_limit = 10

    for item_id in last_items:
        recs = get_item_recommendations(item_id, limit=per_item_limit) 
...
def get_recommendations(
    ...
    try:
        ...
        if user_id:
            ...
            track_recommendation_type('user_personalized')
        else:
            result = get_item_recommendations(item_id, limit)
            track_recommendation_type('item_similar')
        
        count = len(result)
        record_item_count(count)
        if count == 0:
            track_empty_response()
